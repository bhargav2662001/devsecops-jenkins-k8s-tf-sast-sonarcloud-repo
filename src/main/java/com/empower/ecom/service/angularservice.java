...
//	    public login updateByEmail( login login) {
//	        Optional<login> existingLogin = cr.findByEmail(login.getemail());
...
//	        }
//	    }
	    public login updateByEmail(String email, String password) {
	        Optional<login> existingLogin = cr.findByEmail(email);
	        if (existingLogin.isPresent()) {
	            login loginToUpdate = existingLogin.get();
	            loginToUpdate.setpassword(password);
	            return cr.save(loginToUpdate);
	        } else {
	            return null;
	        }
	    }
